...
const links = [{
  ...
  color: 'fff',
  href: 'https://github.com/gyfis'
}, {
  gradient: 'Facebook Messenger',
  icon: 'fa fa-2x fa-facebook',
  color: 'fff',
  href: 'https://www.facebook.com/Tomas.Hromada'
}, {
  gradient: 'Reef',
  icon: 'fa fa-2x fa-twitter',
  color: 'fff',
  href: 'https://twitter.com/gyfis'
}, {
  gradient: 'Politics',
  icon: 'fa fa-2x fa-linkedin',
  color: 'fff',
  href: 'https://www.linkedin.com/in/tom%C3%A1%C5%A1-hromada-8568344a'
}, {
  gradient: 'Juicy Orange',
  icon: 'fa fa-2x fa-stack-overflow',
  color: 'fff',
  href: 'http://stackoverflow.com/users/1581558/gyfis'
}, {
  gradient: 'Deep Space',
  icon: 'fa fa-2x fa-steam',
  color: 'fff',
  href: 'https://steamcommunity.com/id/gyfis'
}];

let randomGradientName = gradients[Math.round(153 * Math.random())].name;
while (links.find(link => link.gradient === randomGradientName)) {
  randomGradientName = gradients[Math.round(153 * Math.random())].name;
}
links.push({
  gradient: randomGradientName,
  icon: 'fa fa-2x fa-envelope-o',
  color: 'fff',
  href: 'mailto:yo@tomashromada.cz',
  type: 'mail'
});
...
for (var x = 0; x < 9; x++) {
  ...
  for (var y = 0; y < 14; y++) {
    ...
    const gradient = gradients[y * 11 + x];
    const angle = Math.round(360 * Math.random());

    const col = document.createElement('div');
    col.className = 'col';
    col.style.backgroundImage = 'linear-gradient(-' + angle + 'deg, ' + gradient.colors[0] + ', ' + gradient.colors[1] + ')';

    const link = links.find(link => link.gradient === gradient.name);
    if (link) {
      const colLink = document.createElement('a');
      colLink.href = link.href;
      if (colLink.type !== 'mail') colLink.target = '_blank';
      colLink.className = 'link';

      const icon = document.createElement('i');
      icon.className = link.icon;
      icon.style.color = link.style;

      colLink.appendChild(icon);
